...
class lif:
    def __init__(self, I, **kwargs):
        #define variables
        self.const_cap = 10
        self.voltage = np.zeros(10000, dtype=float)
        self.resistance = 1
        self.threshold = 1
...
def plot_potential_decay(obj):
    ...
    for i in range(0, len(arr_time) - 1):
        ...
        obj.voltage[i+1] = obj.voltage[i] + obj.mini_time*((obj.I[i] - (obj.voltage[i]/obj.resistance))/obj.const_cap)
...
def count_spikes(obj):
    ...
    for i in range(0, len(arr_time) - 1):
        ...
        obj.voltage[i + 1] = obj.voltage[i] + obj.mini_time * ((obj.I - (obj.voltage[i] / obj.resistance)) / obj.const_cap)
...
def plot_spiking_behavior():
    arr_current = np.arange(0, 50, 0.1)
    num_spikes = []
